...
export function useRecognitionRuntime({
  ...
  useEffect(() => {
    setPrediction((current) => {
      const hasHand = !!latestHandFrame?.hasHand;
      return current.hasHand === hasHand ? current : { ...current, hasHand };
    });
    ...
    void processStreamingHandFrame(latestHandFrame, {
      ...
      setLastHandedness: (handedness: string | null) =>
        setPrediction((current) => {
          const typedHandedness = handedness as PredictionViewModel["handedness"];
          return current.handedness === typedHandedness
            ? current
            : { ...current, handedness: typedHandedness };
        }),
      setLastLabel: (label: string) =>
        setPrediction((current) =>
          current.label === label ? current : { ...current, label }
